...
class DbUtil {
  static Future<bool> deleteCachedPagesOlderThanSetCacheTime() async{
    try {
      ...
      if(Util.isValid(cacheTime)){
        int lastTime = DateTime.now().subtract(Duration(seconds: cacheTime!)).millisecondsSinceEpoch;
        ServeDynamicUIDatabase serveDynamicUIDatabase = await ServeDynamicUIDatabaseProvider.instance.database;
        int? deletedPages = await serveDynamicUIDatabase.cachedPageDao.deleteCachedPagesOlderThanTime(lastTime);
  ...
  static Future<bool> deleteCachedPageIfOlderThanSetCacheTime(String pageKey) async{
    try {
      int? cacheTime = await Util.getPageCacheTime();
      if(Util.isValid(cacheTime)){
        int lastTime = DateTime.now().subtract(Duration(seconds: cacheTime!)).millisecondsSinceEpoch;
        ServeDynamicUIDatabase serveDynamicUIDatabase = await ServeDynamicUIDatabaseProvider.instance.database;
        int? deletedPage = await serveDynamicUIDatabase.cachedPageDao.deleteCachedPagesOlderThanTime(lastTime);
